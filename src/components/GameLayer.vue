...
function updateCharacterPosition(deltaTime: number) {
  if (character.isSleeping) return;
...
function checkCollisions() {
  ...
  for (const bed of beds) {
    ...
    if (
      ...
      if (minOverlap === overlapLeft) character.x = expandedBed.x - character.width;
      else if (minOverlap === overlapRight) character.x = expandedBed.x + expandedBed.width;
      else if (minOverlap === overlapTop) character.y = expandedBed.y - character.height;
      else if (minOverlap === overlapBottom) character.y = expandedBed.y + expandedBed.height;
    }
  }
}
...
function handleBedClicked(bedId: number) {
  ...
  const distance = Math.sqrt(
    Math.pow(characterCenterX - bedCenterX, 2) + 
    Math.pow(characterCenterY - bedCenterY, 2)
  );
  ...
  >
    ...
    >
      ...
      <BedSvg
        ...
        :width="bed.width"
        :height="bed.height"
        :isOccupied="bed.id === character.currentBedIndex && character.isSleeping"
        @bedClicked="handleBedClicked"
        @dragStart="handleBedDragStart"
        @dragMove="handleBedDragMove"
        @dragEnd="handleBedDragEnd"
      />
