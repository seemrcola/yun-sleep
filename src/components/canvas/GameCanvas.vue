...
function checkBedCollisions() {
  for (const bed of beds) {
    ...
    if (
      ...
      if (!bed.isDragging) {
        if (minOverlap === overlapLeft) player.x = expandedBed.x - player.width;
        else if (minOverlap === overlapRight) player.x = expandedBed.x + expandedBed.width;
        else if (minOverlap === overlapTop) player.y = expandedBed.y - player.height;
        else if (minOverlap === overlapBottom) player.y = expandedBed.y + expandedBed.height;
      }
    }
  }
}
    ...
    for (let i = 0; i < beds.length; i++) {
      ...
      if (isPointInBed(x, y, bed)) {
        ...
        const distance = Math.sqrt(
          Math.pow(playerCenterX - bedCenterX, 2) + 
          Math.pow(playerCenterY - bedCenterY, 2)
        );
